...
class VoiceCommandProcessor(
    ...
    fun processCommand(spokenText: String) {
        CoroutineScope(Dispatchers.IO).launch {
            ...
            val cleanedText = removeKeywords(spokenText, categoryKeywords + courseKeywords + sectionKeywords + quizKeywords + lectureKeywords + resourceKeywords)

            // Now, search with cleaned text
            val searchResult = when {
                categoryKeywords.any { spokenText.contains(it, ignoreCase = true) } -> searchCategory(cleanedText)
                courseKeywords.any { spokenText.contains(it, ignoreCase = true) } -> searchCourse(cleanedText)
                sectionKeywords.any { spokenText.contains(it, ignoreCase = true) } -> searchSection(cleanedText)
                quizKeywords.any { spokenText.contains(it, ignoreCase = true) } -> searchQuiz(cleanedText)
                lectureKeywords.any { spokenText.contains(it, ignoreCase = true) } -> searchLecture(cleanedText)
                resourceKeywords.any { spokenText.contains(it, ignoreCase = true) } -> searchResource(cleanedText)
            ...
            searchResult?.let { route ->
                ...
                val errorMessage = when {
                    ...
                    spokenText.contains("resource", ignoreCase = true) -> "Sorry, no resource found."
                    spokenText.contains("category", ignoreCase = true) -> "Sorry, no category found."
